"""
grade_signal.py — Setup graderingssystem A/B/C
Verificeret: A-grade alene gav 62.8% WR og Sharpe 5.19 i backtest

Inverterede tærskler: moderate pumps med lav RSI = bedst (A).
  A (score ≥8): 7% risiko  
  B (score 5-7): 5% risiko
  C (score <5): Skip
"""

GRADE_RISK = {"A": 0.07, "B": 0.05, "C": 0.03}
SKIP_GRADES = {"C"}
...
def grade_signal(pump_pct, rsi, entry_price, pump_high,
    ...
    # 1. Pump størrelse — moderat 20-35% er bedst
    if 20 <= pump_pct < 35:    s_pump = 2
    elif 35 <= pump_pct < 60:  s_pump = 1
    else:                       s_pump = 0
    score += s_pump

    # 2. RSI — lav RSI er bedst for short
    if rsi < 65:    s_rsi = 2
    elif rsi < 75:  s_rsi = 1
    else:           s_rsi = 0
    score += s_rsi

    # 3. Volumen spike — moderat er bedst
    vol = pump_volume / avg_volume if avg_volume > 0 else 1.0
    if 1.5 <= vol < 2.5:   s_vol = 2
    elif 2.5 <= vol < 4.0: s_vol = 1
    else:                   s_vol = 0
    score += s_vol

    # 4. Entry fra pump top — 3-12% fra top er bedst
    from_top = (pump_high - entry_price) / pump_high * 100 if pump_high > 0 else 99
    if 3 <= from_top <= 12:                        s_entry = 2
    elif from_top < 3 or 12 < from_top <= 20:      s_entry = 1
    else:                                           s_entry = 0
    score += s_entry

    # 5. ATR/pris ratio — 1.5-5% er bedst
